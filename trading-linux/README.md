...
- **Wayland Graphics:** Weston compositor with NVIDIA proprietary driver support
...
See [docs/TRADINGOS.md](docs/TRADINGOS.md) for detailed deployment instructions, including:
...
TradingOS uses **Weston** as the primary Wayland compositor for the following reasons:

- **NVIDIA Compatibility:** Weston works correctly with NVIDIA proprietary drivers
- **Window Management:** Desktop shell provides minimize, maximize, and close functionality
- **Lightweight:** Minimal overhead suitable for trading systems
- **Stability:** Proven reliability for embedded and real-time applications

**Removed:** Sway compositor was removed due to incompatibility with NVIDIA proprietary drivers. Sway requires wlroots which has known EGL issues with NVIDIA's implementation.

See [overlay/opt/trading/docs/WAYLAND_COMPOSITORS.md](buildroot-external/board/trading/overlay/opt/trading/docs/WAYLAND_COMPOSITORS.md) for detailed compositor configuration and alternatives.
...
**November 2025 - Wayland Compositor Update:**
- Removed Sway compositor due to NVIDIA proprietary driver incompatibility
- Configured Weston as primary Wayland compositor with desktop shell
- Removed wlroots dependency (only required for Sway)
- Updated systemd services and configuration scripts
- Added documentation for compositor selection and troubleshooting

**Root Cause:** Sway relies on wlroots which has EGL compatibility issues with NVIDIA's proprietary driver implementation. Weston provides equivalent functionality with full NVIDIA support.

**Impact:** System now uses Weston for all Wayland applications. Window management functionality remains available through Weston's desktop shell.

---

**Detailed Documentation:** See [docs/TRADINGOS.md](docs/TRADINGOS.md) for complete technical specifications, kernel configuration, package details, and deployment procedures.
